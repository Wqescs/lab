...
class ArrayInterpreter:
    ...
    def rand(self, name, count, ll, rr):
        if name in self.arrays:
            try:
                for _ in range(count):
                    self.arrays[name].append(str(random.randint(ll,rr)))
    ...
    def stats(self, name1):
        if name1 in self.arrays:
            try:
                array = np.array(self.arrays[name1], dtype=float)
                mean = np.mean(array)
...
intepreter.load_array("A", "lab2.txt")
intepreter.load_array("B", "lab2.txt")
intepreter.mix("B")
intepreter.print_all("A")
intepreter.print_all("B")
intepreter.concatenate("A", "B")
intepreter.print_all("A")
intepreter.save_array("B", "lab2.txt")
